...
docsApp.serviceFactory.sections = function sections() {
    ...
    angular.forEach(DOC_DATA, function(page) {
        ...
        page.partialUrl = (window.useHtml5Mode ? '' : '#') + 'documentation/partials/' + page.group + '/' + page.section + '/' +  page.id + '.html';
        ...
        if (DOCS_PATH.test(path)) {
            ...
            if(isGroupIndexPage){
                ...
                if (partialId) {
                    ...
                    if(partialId != 'index') {
                        breadcrumb.push({ name: sectionName, url: (window.useHtml5Mode ? '' : '#') + 'documentation/' + groupId + '/' + sectionId });
                    } else {
