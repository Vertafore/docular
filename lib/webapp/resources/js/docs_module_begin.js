    ...
    return {
        ...
        template: [
            '<div class="documentation-groups">',
                '<div ng-repeat="group in docGroups" class="documentation-group">',
...
docsApp.directive.documentationSectionList = function($timeout) {
    return {
        ...
        template: [
            '<div class="section-group">',
                '<div class="hero-unit" ng-show="showHeader">',
                    '<h2><i class="{{groupInfo.groupIcon}} icon-white"></i> {{groupInfo.title}} </h2>',
                '</div>',
                '<ol class="rounded-list">',
                    '<li ng-repeat="section in groupInfo.sections" class="documentation-group-section">',
                        '<a href="{{section.url}}">{{section.title}}</a>',
                    '</li>',
                '</ol>',
            '</div>'
        ...
        link: function(scope, element, attrs) {
            ...
            scope.groupId = attrs.group ? attrs.group : scope.$parent.currentGroupId;
            scope.showHeader = attrs.header === "false" ? false : true;

            var getGroup = function(oldValue, newValue){
                console.log("NEW VALUE", newValue, scope.groupId, scope.$parent.currentGroupId);
                var allGroups = scope.$parent.docGroups;
                for(var i=0; i < allGroups.length; i++) {
                    if(allGroups[i].id == scope.groupId) {
                        scope.groupInfo = allGroups[i];
                    }
                }
            };

            scope.$watch('groupId', getGroup);
        }
    };
};
    ...
    var sections = {
        ...
        getGroups : function () {
            ...
            for(var j=0; j < GROUP_DATA.length; j++){
                ...
                sections.groupMap[group.groupId] = {groupTitle:group.groupTitle, sections:{}, groupIcon:group.groupIcon};
    ...
    $scope.$watch(function docsPathWatch() {return $location.path(); }, function docsPathWatchAction(path) {
        ...
        if (DOCS_PATH.test(path)) {
            ...
            if(isGroupIndexPage){
                ...
                //expose some information for this mode
                for(var i=0; i < $scope.docGroups.length; i++){
                    if($scope.docGroups[i].id == groupId){
                        $scope.groupInfo = $scope.docGroups[i];
                    }
                }

                console.log("MODE CHANGED:", $scope.currentGroupId);
