    ...
    return {
        ...
        template: [
            '<div class="view-source" ng-show="currentSource.source">',
                '<span class="btn btn-primary"><i class="icon-spinner icon-spin"></i> view source</span>',
            '</div>'
        ].join(''),
        link: function(scope, element, attrs) {
            ...
            element.bind('click', function(){
                ...
                var highlight = scope.currentSource.lineStart + '-' + scope.currentSource.lineEnd;

                //let's determine if this is a doc file that doesn't need syntax highlighting
                var language = "";
                if(scope.currentSource.source.contentURL.indexOf('.doc.txt') != -1 || scope.currentSource.source.contentURL.indexOf('.ngdoc.txt') != -1){
                    language = "plain-text";
                }
        ...
        template: [
            '<div class="view-source hide-source">',
                '<span class="btn source-file">',
                    ...
                    '<span class="btn btn-primary" style="margin-left: 10px;">hide source</span>',
                '</span>',
            '</div>'
        ].join(''),
        link: function(scope, element, attrs) {
        ...
        if (DOCS_PATH.test(path)) {
            ...
            if(isGroupIndexPage){
                ...
                $scope.currentSource = {
                    ...
                    lineStart: $scope.currentPage.lineStart,
                    lineEnd: $scope.currentPage.lineEnd
                };
