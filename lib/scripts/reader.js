...
function collect(sources, docs_api_in) {
        ...
        files.forEach(function(file) {
            ...
            if(/\.js$/.test(file)) {
                work = Q.when(qfs.read(file, 'b'), function(content) {
                    processJsFile(content, file, sources.group, sources.section, docs_api).forEach (function(doc) {
                        allDocs.push(doc);
                    });
                });
            }
    ...
    var getDOCFiles = function(files){
        ...
        files.forEach(function(file) {
            var work;
            if (file.match(/^.*\.(doc|ngdoc)$/i)) {// here we match on ngdoc to support angular, but we should just use .doc files
                work = Q.when(qfs.read(file, 'b'), function(content){

                    //if we have a file with extension matching .ngdoc or .doc
                    if(file.match(pattern_docFile_angular) || file.match(pattern_docFile_default)){
                        try{
                            processDocFile(content, file, sources.group, sources.section, docs_api).forEach (function(doc) {
                                allDocs.push(doc);
                            });
                        } catch (e) {
...
function processDocFile(content, file, group, section, docs_api) {
...
function processJsFile(content, file, group, section, docs_api) {
