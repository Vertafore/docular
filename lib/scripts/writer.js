...
exports.makeDir = function(p, relative) {
    ...
    var checkExists = function (pt, callBack) {
        return qfs.exists(pt).then(function(exists){
            callBack(exists);
        });
    };

    // Recursively rebuild directory structure (this seems broken by the way)
    return qfs.exists(p).then(function createPart(exists) {

        if(!exists && parts.length) {
            path += "/" + parts.shift();
            return checkExists(path, function(exists) {
                if (!exists) {
