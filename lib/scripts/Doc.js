...
Doc.prototype = {
    ...
    getModuleInfo: function () {
        ...
        console.log(str);
        var match;
        var moduleInfo = {module: null, section: null, item: null, subItem: null};
        
        if(this.docType == 'input') {
            this.docType = 'directive';
        }
        
        if(match = str.match(/^(.*)[\\._]+/)) {
            moduleInfo.module = match[1];
            var modulePieces = moduleInfo.module.split('.');
            
            //Module pieces length may change - do not store reference
            for(var mi = 0; mi < modulePieces.length; mi++) {
                for(var i = 0, l = specialNames.length; i < l; i++) {
                    if(modulePieces[mi] == specialNames[i]) {
                        modulePieces.splice(mi);
                        mi--;
                    }
                }
            }
            moduleInfo.module = modulePieces.join('.');
        } else {
            /**
             * If this is something that cannot have child methods, it's an angular thing.
             */
            if(this.docType == 'directive' || this.docType == 'function' || this.docType == 'filter') {
                moduleInfo.module = 'angular';
            } else {
                moduleInfo.module = str;
            }
        }
        
        if(moduleInfo.module.indexOf('Provider')) {
            moduleInfo.module = moduleInfo.module.replace('Provider', '');
        }
        
        
        console.log("Module Name: " + moduleInfo.module)
        
        moduleInfo.section = this.docType;
        if(!moduleInfo.section) {
            if(match = str.match(/^.*\.(.*)[_]/)) {
                moduleInfo.section = match[1];
            }
            if(!moduleInfo.section && (match = str.match(/^.*\.(.*)/))) {
                moduleInfo.section = match[1];
            }
        }
        
        console.log("Module Section: " + moduleInfo.section)
        
        if(match = str.match(/^.*[_]([^_]+)/)) {
            moduleInfo.item = match[1];
        }
        if(!moduleInfo.item && (match = str.match(/^.*\.([^\._]+)$/))) {
            moduleInfo.item = match[1];
            if(this.docType != 'function') {
                moduleInfo.module = moduleInfo.module + '.' + match[1];
            }
        }
        if(!moduleInfo.item && moduleInfo.section === this.docType) {
            if(moduleInfo.module.indexOf('.') == -1) {
                moduleInfo.item = str;
            }
        }
    ...
    */
    ...
    parse: function() {
        ...
        var processKeyValuePair = function (atKey, atValue) {
            atKey != 'description' && console.log(atKey, atValue);
            var match;
