    ...
    // we need to copy over all UI resources from the doc apis and generate the index.html page with the doc_api resources injected
        ...
        for(var plugin in ui_plugins) {
            if (ui_plugins.hasOwnProperty(plugin)){
                ...
                ui_plugins[plugin].css = ui_plugins[plugin].css || [];
                ui_plugins[plugin].javascript = ui_plugins[plugin].javascript || [];

                for(i=0; i < ui_plugins[plugin].css.length; i++){
                    sourceFileName = ABS_NODE_MODULES + '/docular-ui-plugin-' + plugin + '/' + ui_plugins[plugin].css[i];
                    filePieces = sourceFileName.split('/');
                    targetFileName = filePieces[filePieces.length-1];
                    uiResourceList.push({
                        type:'css',
                ...
                for(i=0; i < ui_plugins[plugin].javascript.length; i++){
                    sourceFileName = ABS_NODE_MODULES + '/docular-ui-plugin-' + plugin + '/' + ui_plugins[plugin].javascript[i];
                    filePieces = sourceFileName.split('/');
                    targetFileName = filePieces[filePieces.length-1];
                    uiResourceList.push({
                        type:'js',
