...
var generate = function (options_in, callBack) {
    ...
    var WEBAPP_FOLDER = path.resolve(ABS_LIB + '/webapp');
    var DEFAULT_WEBAPP_DOCUMENTATION_FOLDER = ABS_LIB + '/generated/webapp';
    var outWebapp = path.resolve(options.docular_webappDir || DEFAULT_WEBAPP_DOCUMENTATION_FOLDER);
    var ABS_NODE_MODULES = ABS_BASE + NODE_MODULES_PATH;
    var GLOBAL_NODE_MODULES = ABS_NODE_MODULES + '/../../';
    ...
    var REL_WEBAPP = path.relative(process.cwd(), outWebapp);
    ...
    var generateSupportingFiles = function (writeFutures) {
        ...
        writeFutures.push(writer.output(outWebapp + '/documentation/docs-metadata.js',
        ...
        writeFutures.push(writer.output(outWebapp + '/documentation/groups-metadata.js',
        ...
        writeFutures.push(writer.output(outWebapp + '/documentation/layout-metadata.js',
    ...
    var generateConfigScript = function (writeFutures, baseUrl) {
        ...
        writeFutures.push(writer.output(outWebapp + '/documentation/docular-configuration.js',[configScript]));
    };
        ...
        var getPartialPromise = function (url, partialName) {
            return Q.when(qfs.read(url, 'b'), function(content) {
                return writer.output(
                    outWebapp + '/resources/docular-partials/'+ partialName +'.html',
                    content.toString()
                );
            });
        };
    ...
    Q.when(loadConfiguredPlugins(), function(content) {}).then(function (){
        ...
        return writer.makeDir(path.resolve(DEFAULT_WEBAPP_DOCUMENTATION_FOLDER), true);

    }).then(function () {

        return writer.copyDir(WEBAPP_FOLDER, outWebapp);
    ...
    //Once some supporting directories are created we should get get all plugins
        ...
        return Q.when(qfs.list(ABS_NODE_MODULES), function(files){
            return Q.when(qfs.list(GLOBAL_NODE_MODULES), function (globalModules) {
    ...
    //then we have all the docs into the docs array so time to do some post processing
        ...
        try {
            ...
            docs.forEach(function(doc){
                ...
                filePromises.push(writer.output(outWebapp + '/documentation/partials/' + doc.group + '/' + doc.section + '/' + id + '.html', doc.html()));
            });
    ...
    // we need to copy over all UI resources from the doc apis and generate the index.html page with the doc_api resources injected
        ...
        function resolvePluginFileName(relativeFilePath) {
            var absNodeModuleFile = path.join(ABS_NODE_MODULES, relativeFilePath),
                globalNodeModuleFile = path.join(GLOBAL_NODE_MODULES, relativeFilePath);
