...
var generate = function (options_in, callBack) {
    ...
    var defaultDocAPI = 'docular-doc-api-doc';
    ...
    writer.makeDir(REL_WEBAPP + '/documentation/partials/', true).then(function(){
        ...
        return Q.when(qfs.listTree(ABS_DOC_APIS), function(files){
            ...
            var rawDocAPIS = {};
            var pattern_apiFile = new RegExp(doc_apis_path + "/" + docularAPIPrefix + "([^\/\\]+)/([^/]+)$",'i');
            ...
            files.forEach(function(file) {
                ...
                if(matches && matches[2] == "index.js"){
                    try{
                        rawDocAPIS[matches[1]] = require(file);
                        doc_apis[matches[1]] = require(file);
                    } catch(e) {
