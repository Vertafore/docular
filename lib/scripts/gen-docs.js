...
var generate = function (options_in, callBack) {
    ...
    var generateSupportingFiles = function (writeFutures) {
        console.log("Running generateSupportingFiles")
    ...
    var generateConfigScript = function (writeFutures, baseUrl) {
        console.log("Running generateConfigScript")
        var configScript = "";
        ...
        if(baseUrl){
            configScript = configScript + "baseURL = '" + baseUrl + "'; useHtml5Mode=" + options.useHtml5Mode.toString() + "; + addTag('base', {href: '" + baseUrl + "'}); ";
        }
        ...
        configScript += "autoloadExamples = " + JSON.stringify(options.examples.autoBootstrap) + ";\n";
        configScript += "exampleJs = " + JSON.stringify(options.examples.include.js.map(pathTrimmer)) + ";\n";
        configScript += "exampleCss = " + JSON.stringify(options.examples.include.css.map(pathTrimmer)) + ";\n";
        configScript += "loadAngular = " + JSON.stringify(options.examples.include.angular) + ";\n";
    ...
    var generateWebappPartials = function (writeFutures) {
        console.log("Running generateWebappPartials")
        var partials = {
            ...
            'docular_partial_documentation' : options.docular_partial_documentation ? options.docular_partial_documentation : ABS_RESOURCES + '/templates/docular_partial_documentation.html',
            'docular_partial_404' : options.docular_partial_404 ? options.docular_partial_documentation : ABS_RESOURCES + '/templates/docular_partial_404.html',
            'example' : options.docular_partial_example || ABS_RESOURCES + '/templates/example.html'
        };
    ...
    // we need to copy over all UI resources from the doc apis and generate the index.html page with the doc_api resources injected
        ...
        fs.mkdirSync(ABS_WEBAPP + '/resources/extra/');
        fs.mkdirSync(ABS_WEBAPP + '/resources/extra/js/');
        fs.mkdirSync(ABS_WEBAPP + '/resources/extra/css/');
        
        for(var i = 0, l = options.examples.include.js.length; i < l; i++) {
            var file = options.examples.include.js[i];
            var fileName = file.split('/');
            fileName = fileName[fileName.length - 1];
            fse.copySync(path.resolve(process.cwd(), file), ABS_WEBAPP + '/resources/extra/js/' + fileName);
        }
        
        for(var i = 0, l = options.examples.include.css.length; i < l; i++) {
            var file = options.examples.include.css[i];
            var fileName = file.split('/');
            fileName = fileName[fileName.length - 1];
