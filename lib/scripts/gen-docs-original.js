...
/*============ BEGIN THE SYNCHRONOUS PROCESS USING ASYCHRONOUS LIBRARY (Q) AS WRAPPERS FOR ASYNCHRONOUS SUB ROUTINES ===========*/
...
//this step scans all documents (ng-docs) and parses their information into an array
}).then(function() {
...
//this step gathers the array of docs and writes each of the partials and other files
}).then(function generateHtmlDocPartials(docs_) {

    //now we have the docs parsed
    docs = docs_;
    ...
    //now we need to generate the HTML for each doc
    ...
    //add the rest of the other expected files (mostly sim linked or copied files)
    writeTheRest(fileFutures);

    return Q.deep(fileFutures);


//then the apcache-offline.manifest is rebuilt
...
//Then we are done!! Print some stats!
}).then(function printStats() {

    console.log('DONE. Generated ' + docs.length + ' pages in ' + (now()-start) + 'ms.' );

}).done();//trigger the Queue!!


//This copies other static files into the build directory or symlinks them to their original spot
function writeTheRest(writesFuture) {
    ...
    //sym link to the original offline.html file
    ...
    //sym link to the original favicon.ico
